...
function cleanString (string) {
  if (string !== undefined) { string = string.trim() }
...
for (const optimize of [false, true]) {
  for (const filename of process.argv.slice(firstFileArgumentIndex)) {
    if (filename !== undefined) {
      ...
      if (!stripSMTPragmas) { input.settings.modelChecker = { engine: 'none' } }
      ...
      { console.log(filename + ': <ERROR>') } else {
        for (const contractFile in result.contracts) {
          for (const contractName in result.contracts[contractFile]) {
            ...
            if ('metadata' in contractResults && cleanString(contractResults.metadata) !== undefined) { metadata = contractResults.metadata }
